...
//The main functioning of the game is here.
const navClick = (text) => {
    switch (text) {
        case "Solvable Game":
...
navBar.element().addEventListener("click", function(event){navClick(event.target.innerText);});
...
const newSolitaire = (type) => {
    ...
    switch(type){
        ...
        case 'replay':
            if(oldDeck !== false){
                let passTheDeck = currentGame().reconstituteDeck(oldDeck);
                currentGame().newGame(passTheDeck);
            } else {
                alert('There is no current game to restart. Please select either a random or solvable game.');
            }
            break;
    }
}
...
let clickEvent = (event) => {
    ...
    let clickClasses = [
        {text:"solveClick",exec:currentGame().quickSolve},
...
//             return currentGame().fireCard(clickedCard);
//         }
//     }
